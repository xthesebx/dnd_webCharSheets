...
public class CharView extends JavalinAuthPage {
    public CharView(Context ctx) throws SQLException, NoSuchAlgorithmException {
        ...
        if (cancel) return;
        String html = FileUtil.readFile("html/viewChar.html");
        ...
        for (int i = 1; i <= rsmd.getColumnCount(); i++) {
            ...
            if (rs.getString(i) == null) {
                html = html.replace("$" + rsmd.getColumnName(i).toUpperCase(), "");
                continue;
            }
            ...
            for (int i = 0; i < 10; i++) {
                if (Mysql.getCharHasSpellLevel(ctx.pathParam("id"), String.valueOf(i))) {
                    ...
                    while (resultSet.next()) {
                        spells.append("<tr><td class=\"tablerow\">" + resultSet.getString("name") + "</td></tr>");
                    }
