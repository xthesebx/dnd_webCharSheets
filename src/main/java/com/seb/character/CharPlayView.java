...
public class CharPlayView extends JavalinAuthPage {
    public CharPlayView(Context ctx) throws SQLException, NoSuchAlgorithmException {
        super(ctx);
        ...
        if (cancel) return;
        String html = FileUtil.readFile("html/playView.html");
        ResultSet rs = Mysql.charDetails(ctx.pathParam("id"));
        ...
        ResultSet weapons = Mysql.getCharWeapons(ctx.pathParam("id"));
        ...
        if (!Mysql.getCharHasSpells(ctx.pathParam("id"))) html = html.replace(" id=\"spellTable\"", " id=\"spellTable\" style=\"display: none\"");
        else {
            for (int i = 0; i < 10; i++) {
                if (Mysql.getCharHasSpellLevel(ctx.pathParam("id"), String.valueOf(i))) {
                    html = html.replaceFirst("spell" + i + "\" style=\"display: none\"", "spell" + i + "\"");
                    ResultSet resultSet = Mysql.getCharSpells(ctx.pathParam("id"), String.valueOf(i));
